...
function updatePhase() {
  ...
  if (time < 0) {
    ...
    narration.textContent = "아래에서 이 조건의 승자와 가상 교전 승리 확률을 확인하세요.";
  }
}
...
function showResult() {
  ...
  document.getElementById("resultValues").hidden = false;
...
function reset() {
  ...
  document.getElementById("resultValues").hidden = true;
...
function frame(timestamp) {
  if (!playing) return;
  if (lastFrame) currentTime = Math.min(model.endTime, currentTime + (timestamp - lastFrame) / 5);
  lastFrame = timestamp;
  render();
